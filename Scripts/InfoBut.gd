...
func _on_InfoBut_button_down():
	yield(get_tree().create_timer(0.1), "timeout")
	if self.pressed:
		get_tree().get_root().get_node("Main/Audio/InfoPop").play()
		...
		self.disabled = false
		#self.texture_normal. сменить фрейм
		PR = 1


		

func _on_InfoBut_button_up():
	...
	if !self.pressed:
		...
		AllButsOff(0)
		if PR:
			get_tree().get_root().get_node("Main/Audio/InfoHide").play()
			PR = 0
